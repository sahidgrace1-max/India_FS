<template>
  <div>
    <h2 class="text-4xl font-extrabold text-center text-blue-900 mb-4">OUR SERVICES</h2>
    <div class="h-1 w-64 bg-green-600 mx-auto mb-12"></div>
    <div v-for="(service, idx) in services" :key="service.title" class="flex flex-col md:flex-row items-center gap-8 mb-16" :class="{ 'md:flex-row-reverse': idx % 2 === 1 }">
      <div class="flex-1 flex justify-center">
        <img :src="service.img" :alt="service.title" class="rounded-lg shadow-lg w-full max-w-md object-cover" />
      </div>
      <div class="flex-1 bg-blue-50 p-8 rounded-lg">
        <h3 class="text-2xl font-semibold mb-4">{{ service.title }}</h3>
        <p class="text-gray-700 leading-relaxed text-justify" style="text-align: justify;">{{ service.desc }}</p>
      </div>
    </div>
  </div>
</template>
