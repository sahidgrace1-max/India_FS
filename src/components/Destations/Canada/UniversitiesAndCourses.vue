      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start">
        ...
        >
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10">
            <div v-for="uni in ['University of Toronto', 'UBC', 'McGill University', 'University of Alberta', 'University of Waterloo']" :key="uni" class="bg-white/70 backdrop-blur-sm border border-blue-100 p-4 rounded-xl shadow-sm flex items-center gap-3">
              <span class="text-green-600 font-bold">#</span>
              <span class="font-bold text-blue-900">{{ uni }}</span>
            </div>
          </div>

          <h2 class="text-2xl sm:text-3xl font-bold text-blue-900 mb-6">Popular Courses</h2>
          <div class="flex flex-wrap gap-3 mb-10">
            <span v-for="course in ['Business & Management', 'IT & Computer Science', 'Engineering', 'Health Sciences', 'Data Science & AI']" :key="course" class="bg-blue-50 text-blue-800 font-bold px-4 py-2 rounded-xl text-sm border border-blue-100">
              {{ course }}
            </span>
          </div>
          ...
          <div class="space-y-4">
            <div class="bg-white/70 backdrop-blur-sm border border-blue-100 p-5 rounded-2xl shadow-sm">
              <p class="font-bold text-blue-900">Vanier Canada Graduate Scholarships</p>
              <p class="text-slate-600 text-sm">Prestigious awards for doctoral students.</p>
            </div>
            <div class="bg-white/70 backdrop-blur-sm border border-blue-100 p-5 rounded-2xl shadow-sm">
              <p class="font-bold text-blue-900">Ontario Graduate Scholarship (OGS)</p>
              <p class="text-slate-600 text-sm">For students studying in Ontario.</p>
            </div>
            <div class="bg-white/70 backdrop-blur-sm border border-blue-100 p-5 rounded-2xl shadow-sm">
              <p class="font-bold text-blue-900">University Entrance & Merit Scholarships</p>
              <p class="text-slate-600 text-sm">Offered by institutions like UofT, UBC, and McGill.</p>
            </div>
          </div>

          <p class="mt-8 text-slate-500 text-sm italic font-medium bg-slate-50 p-4 rounded-xl border border-slate-100">
            Tip: Strong IELTS, PTE, or TOEFL scores improve chances of receiving scholarships at top Canadian universities.
          </p>
        </div>
      </div>
    </div>
