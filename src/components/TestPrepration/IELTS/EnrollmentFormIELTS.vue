<template>
  <section ref="sectionRef" class="relative py-16 lg:py-24 bg-gradient-to-br from-slate-50 via-blue-50/30 to-white overflow-hidden font-poppins">
    ...
    <div class="relative z-10 max-w-2xl mx-auto px-4 sm:px-6">
      ...
      <div 
        class="bg-white/70 backdrop-blur-sm border border-blue-100 rounded-3xl p-8 shadow-sm transition-all duration-1000 ease-out transform"
        ...
        <form @submit.prevent="handleSubmit" class="space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <select v-model="formData.timing" required class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-800/30 focus:border-blue-800 transition-all duration-300 shadow-sm">
            ...
            <div>
              ...
              <select v-model="formData.branch" required class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-800/30 focus:border-blue-800 transition-all duration-300 shadow-sm">
          ...
          <p v-if="successMessage" class="text-green-600 text-sm font-semibold text-center">{{ successMessage }}</p>
          <p v-if="errorMessage" class="text-red-600 text-sm font-semibold text-center">{{ errorMessage }}</p>
        </form>
      </div>
    </div>
  </section>
...
const handleSubmit = async () => {
  ...
  if (!formData.full_name.trim() || !formData.email.trim() || !formData.contact_number.trim() || !formData.timing || !formData.branch || !formData.agree_terms) {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      Object.assign(formData, { full_name: "", email: "", contact_number: "", country: "", timing: "", branch: "", allow_contact: false, agree_terms: false });
    }
  } catch (error) {
